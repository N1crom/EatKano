(function(w) {
    ...
    w.shareText = function(score) {
        ...
        SubmitResults();
        if (score <= 49) return 'Novice';
        if (score <= 99) return 'Regular';
        if (score <= 149) return 'Veteran';
        if (score <= 199) return 'Legend';
        return '???';
    }
    ...
    w.initSetting = function() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "猎杀Tank";
        if (cookie("keyboard")) {
